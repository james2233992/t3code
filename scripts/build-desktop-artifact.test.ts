...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Fenix Code (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Fenix Code (Nightly)");
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.equal(mac.appId, "com.aiworks.fenixcode");
      assert.equal(mac.productName, "Fenix Code (Alpha)");
      assert.equal(mac.artifactName, "Fenix-Code-${version}-${arch}.${ext}");
      assert.equal(linux.appId, "com.aiworks.fenixcode");
      assert.equal((linux.linux as Record<string, unknown>).executableName, "fenixcode");
      assert.deepStrictEqual(
        ((linux.linux as Record<string, unknown>).desktop as Record<string, unknown>).entry,
        { StartupWMClass: "fenixcode" },
      );
      // Linux must register the renderer schemes so the generated .desktop
      // entry advertises MimeType=x-scheme-handler/fenixcode; for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Fenix Code", schemes: ["fenixcode", "fenixcode-dev"] },
      ]);
      assert.equal(win.appId, "com.aiworks.fenixcode");
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.aiworks.fenixcode</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.aiworks.fenixcode");
