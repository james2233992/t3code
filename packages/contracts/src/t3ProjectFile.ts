...
/** File name of the checked-in Fenix project file, resolved at the workspace root. */
export const T3_PROJECT_FILE_NAME = "t3.json";

/** Public URL of the published JSON Schema for {@link T3ProjectFile}. */
export const T3_PROJECT_FILE_SCHEMA_URL = "https://iaonline.io/schema/t3.json";
...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the Fenix Code scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a Fenix Code terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into Fenix Code.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before Fenix Code\'s built-in icon locations.',
      },
  ...
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description:
          "Project scripts shared with everyone who opens this repository in Fenix Code.",
      })
      .check(Schema.isMaxLength(T3_PROJECT_FILE_MAX_SCRIPTS)),
  ),
}).annotate({
  title: "Fenix project file",
  description:
    "Checked-in project configuration for Fenix Code (t3.json at the repository root). See https://iaonline.io for documentation.",
});
