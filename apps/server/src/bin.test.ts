...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("exposes service lifecycle commands without Fenix Connect configuration", () =>
    Effect.gen(function* () {
  ...
  it.effect("reports actionable human-readable headless connect state", () =>
    Effect.gen(function* () {
      ...
      assert.include(output, "Fenix Connect\n  Exposure: disabled");
      assert.include(output, "  Authorization: missing");
      assert.include(output, "  Environment link: not provisioned");
      assert.include(
        output,
        "Next: Run `fenix-code connect link` to authorize and enable Fenix Connect.",
      );
    }),
  );
  ...
  it.effect("disables headless connect without a running server", () =>
    Effect.gen(function* () {
      ...
      assert.equal(output, "Fenix Connect is disabled locally.");
    }),
  );
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["fenix-code", "auth", "pairing", "create"]);
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["fenix-code", "project", "add"]);
