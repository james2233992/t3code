// This file mostly exists because we want dev mode to say "Fenix Code (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Fenix Code (Dev)" : "Fenix Code (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.aiworks.fenixcode.dev.${devBundleIdSuffix || "local"}`
  : "com.aiworks.fenixcode";
export const APP_PROTOCOL_SCHEMES = isDevelopment ? ["fenixcode-dev"] : ["fenixcode"];
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Fenix Code name.
