...
describe("hostedPairing", () => {
  ...
  it("reads hosted pairing host and query token parameters", () => {
    const url = new URL("https://code.iaonline.io/pair?host=100.64.1.2:3773&token=ABCD1234");
  ...
  it("prefers hash tokens so generated hosted links do not put credentials in search params", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://preview.code.iaonline.io");
    ...
    expect(url.origin).toBe("https://preview.code.iaonline.io");
  ...
  it("builds hosted channel selection URLs through the configured router origin", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://code.iaonline.io");
    ...
    expect(url.origin).toBe("https://code.iaonline.io");
    expect(url.pathname).toBe("/__fenixcode/channel");
  ...
  it("ignores incomplete hosted pairing requests", () => {
    expect(
      hasHostedPairingRequest(new URL("https://code.iaonline.io/pair?host=backend.example.com")),
    ).toBe(false);
    expect(hasHostedPairingRequest(new URL("https://code.iaonline.io/pair?token=ABCD1234"))).toBe(
      false,
    );
  });
  ...
  it("detects the hosted static app only when no backend URL is configured", () => {
    ...
    expect(isHostedStaticApp(new URL("https://preview.code.iaonline.io/"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://preview.code.iaonline.io/pair"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://backend.example.com/"))).toBe(false);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://preview.code.iaonline.io/"))).toBe(false);
  });
  ...
  it("detects hosted channel aliases as static apps", () => {
    ...
    expect(isHostedStaticApp(new URL("https://nightly.code.iaonline.io/"))).toBe(true);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://nightly.code.iaonline.io/"))).toBe(false);
  });
});
