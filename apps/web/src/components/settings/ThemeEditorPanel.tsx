...
// A draft with no source theme starts as the standard Fenix Code look — the
...
export function ThemeEditorPanel({
  ...
  const handleSubmit = useCallback(() => {
    if (!name.trim()) {
      setError("Asigna primero un nombre al tema.");
      return;
    }
    ...
    try {
      ...
      if (
        ...
        setError("El tema se guardó, pero no se pudo activar. Inténtalo de nuevo.");
        return;
      }
      onOpenChange(false);
    } catch (cause) {
  ...
  const renderNameField = () => (
    <label className="grid grid-cols-[minmax(0,1fr)_minmax(0,2fr)] items-center gap-3">
      ...
      <Input
        ...
        placeholder={isEditing ? "Nombre del tema" : "p. ej., Aurora"}
        value={name}
      />
    </label>
  );
  ...
  const renderAppearanceButtons = () => (
    <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,2fr)] items-center gap-3">
      ...
      <div aria-label="Apariencia del tema" className="grid grid-cols-2 gap-2" role="group">
  ...
  const renderSidebarArtworkToggle = () => (
    <label className="grid cursor-pointer grid-cols-[minmax(0,1fr)_minmax(0,2fr)] items-center gap-3">
      <span className="text-sm font-medium">Ilustración de la barra lateral</span>
      <span className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
        <span>Mostrar la ilustración del entorno de Fenix Code</span>
        <Switch
          aria-label="Permitir ilustración en la barra lateral con este tema"
  ...
  const renderColorsHeader = () => (
    <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,2fr)] items-start gap-3">
      <div>
        ...
        {isAdvanced ? null : (
          <p className="text-xs text-muted-foreground">Dos colores; el resto se deriva</p>
        )}
      </div>
      <div className="flex min-w-0 items-start gap-3">
        {isAdvanced ? (
          <Input
            aria-label="Filtrar colores"
        ...
        <label className="ml-auto flex shrink-0 cursor-pointer items-center gap-2 pt-0.5 text-sm font-medium">
          <span>Avanzado</span>
          <Switch
            aria-label="Usar colores avanzados del tema"
            checked={isAdvanced}
      ...
      <div className="grid gap-1">
        {THEME_EDITOR_SIMPLE_ROLES.map((role) => (
          <ThemeColorField
            ...
            onToggleSelected={toggleThemeRole}
            role={role}
            label={role === "canvas" ? "Fondo" : "Acento"}
            selected={selectedRole === role}
            value={colorsByAppearance[activeAppearance][role]}
          />
        ))}
  ...
  return (
    <div
      aria-label={isEditing ? "Editar tema" : "Crear tema"}
      className={cn(
    ...
    >
      ...
      >
        <div className="flex min-w-0 flex-1 items-baseline gap-2">
          ...
          {isMinimized ? null : (
            <p className="truncate text-xs text-muted-foreground">
              {isInspecting
                ? "Selecciona un elemento · Esc para cancelar"
                : selectedRole
                  ? `${getThemeRoleLabel(selectedRole)} · ${usageCount ?? 0} ${usageCount === 1 ? "uso" : "usos"}`
                  : "Selecciona un color abajo"}
            </p>
          )}
        </div>
        <Tooltip>
          <TooltipTrigger
            render={
              <Button
                aria-label={
                  isInspecting
                    ? "Cancelar inspección de colores"
                    : "Inspeccionar colores de la aplicación"
                }
              ...
              >
                <MousePointer2Icon />
                {isInspecting ? "Cancelar" : "Inspeccionar"}
              </Button>
            }
          />
          <TooltipPopup data-theme-editor-panel="">
            {isInspecting ? "Cancelar y borrar la selección" : "Elegir un color de la aplicación"}
          </TooltipPopup>
        </Tooltip>
        <Button
          aria-label={isMinimized ? "Expandir el editor de temas" : "Minimizar el editor de temas"}
          size="icon-xs"
          variant="ghost"
        ...
        <Button
          aria-label="Cerrar el editor de temas"
          size="icon-xs"
          variant="ghost"
        ...
        <>
          ...
          <div className="flex items-center justify-end gap-2 border-t border-border/70 px-3 py-2">
            ...
            <Button disabled={!name.trim()} size="sm" onClick={handleSubmit}>
              {isEditing ? (
                mergeTarget ? (
                  `Combinar con «${mergeTarget.label}»`
                ) : (
                  "Guardar cambios"
                )
              ) : mergeTarget ? (
                <>
                  <PlusIcon />
                  {`Añadir paleta ${activeAppearance === "light" ? "clara" : "oscura"}`}
                </>
              ) : (
                <>
                  <PlusIcon />
