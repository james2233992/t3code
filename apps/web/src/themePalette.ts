...
export const T3_CHAT_THEME_LABEL = "Fenix Chat";
...
// Older builds stored the dark Fenix Chat palette as a separate theme. Keep
...
// pixels users see after Fenix Chat's blur and noise layers are composited.
const T3_CHAT_LIGHT_COLORS: ThemeColors = {
  canvas: "#fdf7fd",
  // Fenix Code's workspace header belongs to the chat panel, so keep it seamless
  // with the light chat canvas rather than mapping it to Fenix Chat's outer shell.
  chrome: "#fdf7fd",
  toolbar: "#fdf7fd",
  toolbarForeground: "#501854",
  toolbarBorder: "#efbdeb",
  // Fenix Chat's light chrome controls sit on its pale gradient-noise surface,
  ...
  // Fenix Chat uses a light lavender code surface in light mode. Keeping the
  // dark plum pair here also leaked the dark palette into Fenix Code's diffs.
  codeBackground: "#f5ecf9",
  codeForeground: "#673c8b",
  // The live sidebar is transparent over Fenix Chat's outer shell. Use that
...
const T3_CHAT_DARK_COLORS: ThemeColors = {
  canvas: "#1f1a24",
  // Fenix Code's workspace header belongs to the chat panel, so keep it seamless
  // with the canvas rather than mapping it to Fenix Chat's outer shell.
  chrome: "#1f1a24",
  toolbar: "#1f1a24",
  ...
  // Cards and panels stay in Fenix Chat's plum surface family. Near-black here
  ...
  // Diffs and file previews are full workspace surfaces in Fenix Code. Keep them
  ...
  // Fenix Chat draws the chat panel edge in this muted pink. The resize rail uses
...
/**
 * The palette Fenix Code wears with no theme installed, captured from the app's
...
/**
 * The standard Fenix Code look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship Fenix Chat palette used to fill roles omitted by theme
 * files.
 */
...
/**
 * The status colors Fenix Code shows without a theme, read from the app's own
...
/**
 * A companion action color in the Fenix Chat mold. This gives send buttons,
